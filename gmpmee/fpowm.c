...
gmpmee_fpowm(mpz_t rop, gmpmee_fpowm_tab table, mpz_t exponent)
{
  ...
  mpz_set_ui(rop, 1);
#ifdef REDC
  if (mpz_tstbit(table->spowm_table->modulus, 0)) {
    gmpmee_redc_encode(rop, rop, table->spowm_table->mrt);
  }
#endif
...
#ifdef REDC
      if (mpz_tstbit(table->spowm_table->modulus, 0)) {
	gmpmee_redc_mod(rop, rop, table->spowm_table->mrt);
      } else {
#endif
...
#ifdef REDC
  if (mpz_tstbit(table->spowm_table->modulus, 0)) {
    gmpmee_redc_decode(rop, rop, table->spowm_table->mrt);
  }
#endif
}
