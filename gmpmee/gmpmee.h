...
/**
 *
 * Copyright 2008 2009 Torbj&ouml;rn Granlund, Douglas Wikstr&ouml;m
 *
 * <p>
 *
 * This file is part of GMP Modular Exponentiation Extension (GMPMEE).
...
#ifdef REDC
  gmpmee_redc_t mrt;      /**< Transformed modulus needed by redc arithmetic. */
#endif
...
/**
 * Equivalent to calling gmpmee_spowm_init and then gmpmee_spowm_precomp.
 *
 * @param table Table to be initialized
 * @param bases Bases for which precomputation is performed.
 * @param len Number of bases in the simultaneous exponentiation.
 ...
 */
void
gmpmee_spowm_init_precomp(gmpmee_spowm_tab table, mpz_t *bases, size_t len,
...
/**
 * Computes a simultaneous exponentiation. Precomputation is performed
 * in blocks of the given width in a single batch.
 *
 * @param rop Destination of result.
 * @param bases Bases for which precomputation is performed.
 * @param exponents Exponents used in simultaneous exponentiation.
 ...
 * factor of the integer has been found or not.
 *
 * @param n Integer to test.
 */
int
...
/**
 * Writes the <code>mpz_t</code> in the array <code>op</code> to the
 * given file in radix <code>radix</code>, one <code>mpz_t</code> per line.
 *
 * @param f Output file descriptor.
 * @param radix Radix used in conversion.
 * @param op Array to be written.
 * @param len Number of elements in array.
 * @param sep Separator character.
 */
void
gmpmee_array_out_str(FILE *f, int radix, mpz_t *op, size_t len, char *sep);
