...
of
\code{vfork.protocol.mixnet.MixNetElGamalInterface}. \sect{inter}
...
\code{vfork.arithm.ModPGroup} and that each plaintext is a
...
\code{vfork.protocol.mixnet} package). This class requires every
...
\code{vfork.crypto.RandomSource} to \vmni{} using the
...
\code{.vfork\_random\_source} in the home directory of the
...
subclass of \code{vfork.crypto.PRG}, then the hexadecimal
encoding of its seed is stored in the file
\code{.vfork\_random\_seed}, or if
...
\code{vfork.ui.gen.Generator} must also be implemented. Please
